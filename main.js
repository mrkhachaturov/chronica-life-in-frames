...
class I18nManager {
    ...
    getRussianPlural(count, one, few, many) {
...
const DEFAULT_SETTINGS = {
    ...
    eventNotesFolder: "",
    weekNoteTemplate: "${gggg}-W${ww}",
    eventNoteTemplate: "${eventName}_${gggg}-W${ww}",
    rangeNoteTemplate: "${eventName}_${start_gggg}-W${start_ww}_to_${end_gggg}-W${end_ww}",
    // --- Filling & Interaction ---
...
function getDefaultEventTypes(i18nManager) {
    return [
        {
...
class ChornicaTimelinePlugin extends obsidian.Plugin {
    ...
    async onload() {
        ...
        this.registerView(CHRONICA_OVERVIEW_VIEW_TYPE, (leaf) => new ChronicaOverviewView(leaf, this));
     ...
     */
    async activateView(viewType = TIMELINE_VIEW_TYPE) {
    ...
    async createOrOpenWeekNote() {
        try {
            ...
            const fileName = this.formatFileName(this.settings.weekNoteTemplate, values);
            ...
            new obsidian.Notice(`Error creating/opening week note: ${error.message}`);
    ...
    getISOWeekYearNumber(date) {
        ...
        const ordinalDayOfThursday = Math.floor((workDate.valueOf() - firstDayOfIsoYear) / 86400000) + 1;
...
class ChornicaEventModal extends obsidian.Modal {
    ...
    eventDescription = "";
    eventName = "";
    selectedTypeId = "preset_major_life";
        ...
        const updateColorIndicatorUI = (typeId) => {
            ...
            if (selectedType) {
                if (selectedType.isPreset) {
                    ...
                    colorIndicator.style.setProperty("--event-type-indicator-color", selectedType.color);
                }
            }
        };
    ...
    async saveEvent() {
        ...
        if (!finalEventName && !finalDescription) {
            ...
            new obsidian.Notice("Please add an event name or description.");
            return;
        }
        if (!this.selectedTypeId) {
        ...
        if (this.isDateRange) {
            ...
            if (endDate < startDate) {
                new obsidian.Notice("End date cannot be before start date. Event not saved.");
                if (rangeValidationMessageEl) {
                    rangeValidationMessageEl.textContent =
                        "End date cannot be before start date.";
                    rangeValidationMessageEl.classList.remove("hidden");
        ...
        if (this.isEditMode && this.originalEvent) {
            ...
            const eventToUpdate = this.plugin.settings.events.find((eventInSettings) => {
                ...
                const periodMatches = original.endWeekKey && original.endWeekKey !== original.weekKey
                ...
                return (periodMatches &&
                    ...
                    eventInSettings.notePath === original.notePath); // Also match notePath if it existed
            });
            if (eventToUpdate) {
                ...
                const noteProcessed = await this.createOrUpdateEventNote(eventToUpdate, // Pass the event object from the settings array
                finalEventName, startDate, endDate);
                ...
                new obsidian.Notice(`Event "${nameForEventObject}" updated.`);
            }
            else {
                new obsidian.Notice("Error: Original event not found for update. Could not save changes. The event might have been modified or deleted externally.");
            ...
            try {
                await this.createOrUpdateEventNote(newEvent, finalEventName, startDate, endDate);
                ...
                new obsidian.Notice("Event saved to settings, but failed to create linked note.");
            }
            this.plugin.settings.events.push(newEvent);
            await this.plugin.saveSettings();
            new obsidian.Notice(`Event "${nameForEventObject}" added.`);
        }
        this.close();
        this.refreshViews();
    }
...
class ChornicaTimelineView extends obsidian.ItemView {
    ...
    renderView() {
        ...
        ["square", "circle", "diamond"].forEach((opt) => {
            const option = shapeSelect.createEl("option", {
                attr: { value: opt },
                text: this.plugin.i18nManager.t().settings[opt],
            });
    ...
    renderWeeksGrid(container) {
        ...
        if (settings.showBirthdayMarker) {
            ...
            if (isPortrait) {
                ...
                birthdayMarkerContainer.style.setProperty("--birthday-marker-top", `${topOffset}px`);
                birthdayMarkerContainer.style.setProperty("--birthday-marker-left", `${leftOffset - 13}px`);
                birthdayMarkerContainer.style.setProperty("--birthday-marker-transform", "translateX(-50%)");
            }
            else {
                // Landscape
                // In landscape, weeks are vertical. Cake should be to the left of the first week row.
                birthdayMarkerContainer.style.setProperty("--birthday-marker-top", `${topOffset + 10}px`);
                birthdayMarkerContainer.style.setProperty("--birthday-marker-left", `${leftOffset - 25}px`);
                birthdayMarkerContainer.style.setProperty("--birthday-marker-transform", "translateY(-50%)");
            }
        ...
        for (let yearIndex = 0; yearIndex < lifespan; yearIndex++) {
            ...
            for (let cellIndex = 0; cellIndex < 52; cellIndex++) {
                ...
                const potentialWeeklyNoteName = this.plugin.formatFileName(settings.weekNoteTemplate, weeklyNotePlaceholderValues);
                // Get full path, false indicates it's NOT an event note for folder purposes
                const potentialWeeklyNotePath = this.plugin.getFullPath(potentialWeeklyNoteName, false);
                ...
                const createTooltipContent = (hoveredCell) => {
                        ...
                        if (!eventNameActual || !isCompact) {
                            ...
                            if (startDateStr && endDateStr) {
                                ...
                                if (startMonth === endMonth &&
                                    ...
                                    endDateStr.includes(" ")) {
                                    formattedCellRange = `${startDateStr} - ${endDateStr.substring(endDateStr.lastIndexOf(" ") + 1)}`;
                                }
                            }
    ...
    renderStatsPanel(container) {
        ...
        statsHandle.setAttribute("title", this.isStatsOpen ? this.plugin.i18nManager.t().stats.hideStatistics : this.plugin.i18nManager.t().stats.showStatistics);
    ...
    renderTimelineTab(container) {
        ...
        addMilestone(this.plugin.i18nManager.t().stats.threeQuarterLife, Math.round(this.plugin.settings.lifespan * 0.75));
        ...
        completionCard.createEl("div", {
            ...
            text: `${filledWeeks} ${this.plugin.i18nManager.t().stats.weeksManuallyAutoFilled} ${this.plugin.i18nManager.t().stats.outOfPastWeeks} ${Math.round(pastWeeks)} ${this.plugin.i18nManager.t().stats.weeksLived} (${completionRate.toFixed(1)}%)`,
        });
        ...
        completionCard.createEl("div", {
            cls: "chronica-completion-stat",
            text: `${totalEvents} ${this.plugin.i18nManager.t().stats.eventsRecorded} (${eventsPerWeek.toFixed(3)} ${this.plugin.i18nManager.t().stats.eventsPerWeek})`,
        });
    }
    /**
    ...
    applyEventStyling(cell, weekKey) {
        ...
        if (!this.plugin.settings.events || !this.plugin.settings.eventTypes) {
            ...
            cell.classList.remove("event", "future-event-highlight", "event-type-custom", "event-unknown-type");
        ...
        let appliedNotePath = undefined; // Keep this for dataset
 ...
 */
class ChronicaOverviewView extends obsidian.ItemView {
    plugin;
    constructor(leaf, plugin) {
        super(leaf);
        this.plugin = plugin;
...
- \${MMM}: Short Month Name (e.g., ${this.plugin.i18nManager.t().months.jan})
  ...
  - \${endDate_YYYY}, \${endDate_MM}, \${endDate_DD}, \${endDate_MMMM}, \${endDate_MMM}, \${endDate_YY} (for actual end date)`;
        ...
        const tooltipDetailSetting = new obsidian.Setting(containerEl) // No need to store this setting object unless used elsewhere
            ...
            .setDesc(this.plugin.i18nManager.t().settings.tooltipDetailLevelDesc);
        ...
        const notePreviewSetting = new obsidian.Setting(containerEl)
            ...
            .setDesc(this.plugin.i18nManager.t().settings.enableNotePreviewInTooltipDesc)
            .addToggle((toggle) => toggle
        ...
        new obsidian.Setting(containerEl)
            ...
            .setDesc(this.plugin.i18nManager.t().settings.manageEventTypesDesc)
            .addButton((button) => {
        ...
        const autoFillToggleSetting = new obsidian.Setting(containerEl)
            ...
            .setDesc(this.plugin.i18nManager.t().settings.enableAutoFillDesc)
            .addToggle((toggle) => toggle
        ...
        const daySelector = new obsidian.Setting(containerEl)
            ...
            .setDesc(this.plugin.i18nManager.t().settings.autoFillDayDesc)
        ...
        const clearFilledSetting = new obsidian.Setting(containerEl) // get a reference to this setting
            ...
            .setDesc(this.plugin.i18nManager.t().settings.clearFilledWeeksDesc)
            .addButton((button) => {
        ...
        new obsidian.Setting(containerEl)
            ...
            button
                ...
                .onClick(async () => {
                if (confirm(this.plugin.i18nManager.t().settings.clearAllEventsConfirm)) {
                    this.plugin.settings.events = []; // Clear the new events array
                    await this.plugin.saveSettings();
                    this.refreshAllViews(); // Update views
                    new obsidian.Notice(this.plugin.i18nManager.t().settings.clearAllEventsNotice);
                }
            });
        });
        ...
        new obsidian.Setting(containerEl)
            ...
            button
                ...
                .setWarning() // Make the button look cautionary
                .onClick(async () => {
                if (confirm(this.plugin.i18nManager.t().settings.resetEventTypesConfirm)) {
                    ...
                    await this.plugin.saveSettings();
                    this.refreshAllViews(); // Update views
                    new obsidian.Notice(this.plugin.i18nManager.t().settings.resetEventTypesNotice);
                }
            });
        });
